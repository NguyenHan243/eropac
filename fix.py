...
WIDTH = 900
HEIGHT = 950
pygame.display.set_caption('Dũng sĩ bảo vệ môi trường')
...
flicker = False # mấy cục nhỏ nhấp nháy
# R, L, U, D
...
image3 = pygame.transform.scale(pygame.image.load(os.path.join('assets/rac/RÁC VÔ CƠ', random.choice(file1))),(40,40))
...
def draw_board():
    ...
    for i in range (len(level)):
        for j in range (len(level[i])):
            ...
            if level[i][j]==11:
                screen.blit(image4,(j*num2 +(0.5*num2) -10, i* num1+(0.5*num1)))
            if level[i][j]==12:
                screen.blit(image5,(j*num2 +(0.5*num2) +15, i* num1+(0.5*num1)))
            if level[i][j]==13:
                screen.blit(image6,(j*num2 +(0.5*num2), i* num1+(0.5*num1)-20))
            if level[i][j]==14:
                screen.blit(image7,(j*num2 +(0.5*num2), i* num1+(0.5*num1)-20))
            if level[i][j]==15:
...
def start_game():
    global run, start_ticks, startup_counter, moving, game_over, game_won, score, level, player, player_x, player_y, direction, direction_command
    run = True
    while run:
        sound_play.play(-1)
...
def play_screen(screen, bg):
    pygame.display.set_caption('Environmental Warrior - Play')
    ...
    while True:
        ...
        play_text = font.render("Are you ready to start?", True, "white")
        play_rect = play_text.get_rect(center=(WIDTH//2, 200))
        ...
        for button in [back_button, ready_button]:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if ready_button.checkForInput(play_mouse_pos):                
                    start_game()
                    return
        pygame.display.flip()

def option_screen(screen, bg):
    pygame.display.set_caption('Environmental Warrior - Rules')
    ...
    while True:
        ...
        rules_text1 = font.render("Collect items while avoiding obstacles", True, "white")
        rules_text2 = font.render("Use arrow keys to move", True, "white")
        rules_text3 = font.render("A, S, D to change characters", True, "white")
        
        screen.blit(rules_text1, (WIDTH//2 - rules_text1.get_width()//2, 200))
        screen.blit(rules_text2, (WIDTH//2 - rules_text2.get_width()//2, 250))
        screen.blit(rules_text3, (WIDTH//2 - rules_text3.get_width()//2, 300))
        ...
        back_button.changeColor(option_mouse_pos)
...
def main_menu(screen, bg):
    ...
    pygame.display.set_caption('Environmental Warrior - Main Menu')
    
    # Use a larger font for menu
    ...
    while True:
        ...
        play_button = Button(
            ...
            font=button_font, 
            base_color='black', 
            hovering_color='green' #đổi màu khi lia chuột
        )
...
#display.flip() sẽ cập nhật nội dung của toàn bộ màn hình
